...
class worker(QtCore.QObject):
    ...
    def constuct_error_map(self, main_graph_data, main_graph_time, x_spin, y_spin, z_spin, comboboxes_text):
        ...
        if comboboxes_text[0] == "number of chunks":
            ...
            if comboboxes_text[1] == "number of chunks":
                ...
                self.construct_matrix_2(1,0)
            else:
                self.operation_number = 4
                self.construct_matrix_2(0,0)
        else:
            if comboboxes_text[1] == "number of chunks":
    ...
    def construct_matrix_2(self,offset_y,offset_x):
        number_of_squares = 0   
        for y_value in range(offset_y,self.y_axis+offset_y):
            for x_value in range(offset_x, self.x_axis+offset_x):
                # print(self.x_axis)
                # print(self.y_axis)
                if self.operation_number == 1:
                    self.fill_matrix(y_value,x_value,0,1,x_value,y_value,self.z_axis)
                elif self.operation_number == 2:
                    self.fill_matrix(y_value,x_value,0,1,x_value,self.z_axis,y_value)
                elif self.operation_number == 3:
                        self.fill_matrix(y_value,x_value,1,0,y_value,x_value,self.z_axis)
                elif self.operation_number == 4:
                        self.fill_matrix(y_value,x_value,0,0,self.z_axis,x_value,y_value)
                elif self.operation_number == 5:
                        self.fill_matrix(y_value,x_value,1,0,y_value,self.z_axis,x_value)
                elif self.operation_number == 6:
                        self.fill_matrix(y_value,x_value,0,0,self.z_axis,y_value,x_value)
                # print(
    ...
    def fill_matrix(self,y_value,x_value,offset_y,offset_x,chuncks,degree,overlap):
        self.construct_chunks_margins(
                    chuncks, self.main_graph_data,self.main_graph_time, overlap)
        self.Matrix[y_value-offset_y][x_value -
                                    offset_x] = self.interpolation_process(degree)
    ...
    def construct_chunks_margins(self, current_number_of_chunks, main_graph_data, main_graph_time, overlap):
        ...
        for chunk in range(0, current_number_of_chunks):
            if (round(((chunk)*self.length_of_chunk), 2)) > (length_of_signal - 0.01):
                left_index = (len(self.main_graph_data) - 1) - (self.overlaped_indices*counter)
            else:
                left_index = self.main_graph_time.index(
                    round((chunk*self.length_of_chunk), 2)) - (self.overlaped_indices*counter)
            if (round(((chunk+1)*self.length_of_chunk), 2)) > (length_of_signal - 0.01):
                right_index = (len(self.main_graph_data) - 1) - (self.overlaped_indices*counter)
            else:
