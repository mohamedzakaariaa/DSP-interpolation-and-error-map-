...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def draw_error_map(self, matrix):
        ...
        self.sc.axes.set_xticks(np.arange(0,self.gui.x_spinbox.value(),int((self.gui.x_spinbox.value())/4)))
        self.sc.axes.set_xticklabels(np.arange(1,(self.gui.x_spinbox.value()+1),int((self.gui.x_spinbox.value()+1)/4)))
        self.sc.axes.set_yticks(np.arange(0,self.gui.y_spinbox.value(),int((self.gui.y_spinbox.value())/4)))
        self.sc.axes.set_yticklabels(np.arange(1,(self.gui.y_spinbox.value()+1),int((self.gui.y_spinbox.value()+1)/4)))
    ...
    def construct_chunks_margins(self):
        ...
        for chunk in range(0, self.number_of_chunks):
            if (round(((chunk)*self.length_of_chunk), 2)) > (self.length_of_signal - 0.01):
            ...
            if (round(((chunk+1)*self.length_of_chunk), 2)) > (self.length_of_signal - 0.01):
    ...
    def LateX(self, array_of_coeff):
        ...
        for i in range(len(array_of_coeff)):
            if i != len(self.main_graph_time)-1:
                text += str("{:.2f}".format(array_of_coeff[i])) + "$X^" + "{"+str(
                    len(array_of_coeff)-1-i)+"}"+"$+"
            else:
                text += str("{:.2f}".format(array_of_coeff[i]))
        
        text = self.adjust_latex(text)
        return text
